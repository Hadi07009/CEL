...
public class SqlgenerateForPayroll
{
    ...
    public static string pfLoanInstallmentUpdateByempid(int monthNo, int yearValue,string empcode)
    {
        string sql = "";
        return sql = @"update b set install_status='DONE' from hrms_pf_loan_hdr a 
                      inner join hrms_pf_loan_det b on a.loan_ref_no=b.loan_ref_no
                      where MONTH([install_date])=" + monthNo + " and YEAR([install_date])=" + yearValue + " and emp_code='" + empcode + "' and install_status='PENDING'";        
    }
    public static string pfLoanInstallmentUpdate(int monthNo, int yearValue)
    {
        string sql = "";
        return sql = @"update b set install_status='DONE' from hrms_pf_loan_hdr a 
                      inner join hrms_pf_loan_det b on a.loan_ref_no=b.loan_ref_no
                      where MONTH([install_date])=" + monthNo + " and YEAR([install_date])=" + yearValue + " and install_status='PENDING'";
    }
