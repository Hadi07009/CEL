...
using System.Data.SqlClient;
    ...
    public class Sqlgenerate
    {
        ...
        {
            return "SELECT DISTINCT EmpTCode, EmpType FROM HRMS_EMP_TYPE ORDER BY EmpType";
        }
        ...
        public static string GettaxbyempidandSalmonth(string empcode,DateTime saldate)
        {
            string sql = "";
            return sql = "select [dbo].[functionTaxcalculationByempid]('" + empcode + "',Convert(Datetime,'" + saldate + "',103)) as Taxamt";
        }

        public static string GetMaxSalDate()
        {
            string sql = "";
            return sql = "select isnull(Dateadd(D,1,MAX(Salmonth)),Getdate()) as NextSaldate from hrms_salary where SalGrade<>'50'";
        }
        ...
        public static string SqlGetPayrollHoldRecord(string employeeCode)
        {
            return "  SELECT * FROM [Hrms_Salary_Hold] WHERE [Empcode] = '" + employeeCode + "' AND convert(DATE,[Holddate], 103)  = convert(DATE, GETDATE(), 103) AND [HoldStatus] = 'Y'";
        }
