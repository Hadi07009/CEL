    ...
    private void Save(string connectionString, AnnualStatement objAnnualStatement)
    {
        try
        {
            ...
            sql = @"INSERT INTO tblAnnualStatementReport
                SELECT A.EmpID
	                ,B.Emp_Mas_TIN
	                ,sum(case when Calcode in('STDBAS','STDOT','BNS1','BNS2','BNS3') then CalVal else 0 end) AS salary
	                ...
	                ,sum(case when Calcode = 'STDENT' then CalVal else 0 end) AS [entertainmentAmount]  
	                ,sum(case when Calcode = 'STDMA' then CalVal else 0 end) AS [Medical Allowance]
	                ,sum(case when Calcode in('STDOTH','STDLFA','STDIB') then CalVal else 0 end) AS [Other Allowance]
