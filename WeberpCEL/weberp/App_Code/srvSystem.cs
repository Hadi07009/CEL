...
public class srvSystem : System.Web.Services.WebService
{
    ...
    // Get Cost Center for Consumption report
     ...
     public string[] GetEmployeeLoan(string prefixText, int count)
     {
         ...
         LoanInformationTableAdapter loan = new LoanInformationTableAdapter();
         dsTransaction.LoanInformationDataTable dtloan = new dsTransaction.LoanInformationDataTable();
         ...
         foreach (dsTransaction.LoanInformationRow dr in dtloan.Rows)
         {
             str[indx] = dr.loan_ref_no.ToString() + ":" + dr.emp_code.ToString() + ":" + dr.EmpName.ToString() + ":" + dr.loan_status;
             indx++;
