...
public partial class ClientSide_modules_mis_naz_FORMS_HRMS_Forms_frmOffdaySpecialApplication : System.Web.UI.Page
{
    ...
    {
        //StaticData.MsgConfirmBox(btnApply, "Are you sure want to Apply Off Day ? ");
        if (!Page.IsPostBack)
        {
    ...
    private void LoadOffdayAttendanceDetailsemployeeID(string empid, string Processid, string flowid, DateTime fDate, DateTime lDate)
    {
        ...
        cmd.CommandType = CommandType.StoredProcedure;
        cmd.CommandText = "spProcessGetOffdayStatusSpecial";
        ...
        try
        {
            ...
            for (int i = 0; i < grdHoliday.Rows.Count; i++)
            {
                ...
                if (chkbox.Checked == true)
                {
                    ...
                    TextBox txtBox = grdHoliday.Rows[i].FindControl("txtNoOfDays") as TextBox;
                    MKB.TimePicker.TimeSelector mkb = (MKB.TimePicker.TimeSelector)grdHoliday.Rows[i].FindControl("timeoff1");
                    MKB.TimePicker.TimeSelector mkb2 = (MKB.TimePicker.TimeSelector)grdHoliday.Rows[i].FindControl("timeoff2");
                    ...
                    offphdr.noOfDays = (float)Convert.ToDouble(txtBox.Text.Trim().ToString());// == "" ? "" : txtBox.Text.Trim().ToString());
            ...
            string retval = lvproc.SaveInitiateOffdayProcessDataSpecial(lvphdrlst, myCommand);
    ...
    protected void grdHoliday_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            TextBox txtBox = e.Row.FindControl("txtNoOfDays") as TextBox;
            double noDays = txtBox.Text.ToString() == "" ? 0 : (float)Convert.ToDouble(txtBox.Text.ToString());
            noOfDays += noDays;
            ...
            Label lbExH = e.Row.FindControl("lblItemExH") as Label;
        ...
        { 
            ...
            e.Row.Cells[12].Text = noOfDays.ToString();
    ...
    protected void btnTotal_Click(object sender, EventArgs e)
    {
        ...
        for (i = 0; i < rowNo; i++)
        {
            ...
            if (action != 0)
            {
                ...
                totalNoOfDays += txtBox.Text.Trim() == "" ? 0 : (float)Convert.ToDouble(txtBox.Text.Trim());
            }
            ...
            rowFooter.Cells[12].Text = totalNoOfDays.ToString();
        }
