...
public partial class ClientSide_modules_mis_naz_FORMS_HRMS_Forms_frmOvertimeApplication : System.Web.UI.Page
{
    string ConnectionStr = System.Configuration.ConfigurationManager.AppSettings["UbasysConnectionString"].ToString();
    string PID = "P004";
    string FID = "4";
    double total = 0;
    const string processType = "OT";
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            dttask = DataProcess.GetData(ConnectionStr, "select * from ProcessTaskDateAllemoloyee where TaskType='OT' and [Status]='Y'");
    ...
    {
        if (Process.GetRateConfigurationStatus(ConnectionStr, processType, Session["ApplicantID"].ToString()) == false)
        {
            ScriptManager.RegisterStartupScript(this, GetType(), "MessageBox", "alert('Overtime amount is not configured yet.Please contact human resource department');", true);
            return;
        }
        ...
        try
        {
            ...
            for (int i = 0; i < grdHoliday.Rows.Count; i++)
            {
                ...
                if (chkbox.Checked == true)
                {
                    ...
                    offphdr.ResponsiblepersonId = ApplicantID.ToString();//dplResponsible.SelectedItem.Value;
            ...
            string retval = lvproc.SaveInitiateOvertimeProcessData(lvphdrlst, myCommand);
            if (retval.ToString() == "")
            {
                myTrans.Rollback("SaveAllTransaction");
            }
    ...
    protected void grdHoliday_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            DateTime dt1 = DateTime.Parse(e.Row.Cells[18].Text.ToString());
            ...
            DateTime dt2 = DateTime.Parse(e.Row.Cells[19].Text.ToString());
            ...
            //string IntimeValue = e.Row.Cells[17].Text.ToString();
            //if (IntimeValue != "&nbsp;")
            //{
            //    DateTime dt = DateTime.Parse(e.Row.Cells[4].Text.ToString());
            //    MKB.TimePicker.TimeSelector.AmPmSpec am_pm;
            ...
            //}
            //string OuttimeValue = e.Row.Cells[18].Text.ToString();
            //if (OuttimeValue != "&nbsp;")
            //{
            ...
            if (e.Row.Cells[3].Text == "P" || e.Row.Cells[3].Text == "A")
            {
                e.Row.Cells[3].Text = "Present";               
            }
            if (e.Row.Cells[12].Text == "00:00")
            {
                chkbox.Checked = false;
            }
            int totalMin = Convert.ToInt32(e.Row.Cells[20].Text.ToString());//just changed the index of cells based on your requirements 
        ...
        {
            e.Row.Cells[11].Text = "Total Extra Hour";
            e.Row.Cells[12].Text = DateProcess.TimeDuration(Convert.ToInt32(total.ToString()));
    ...
    protected void btnTotal_Click(object sender, EventArgs e)
    {
        ...
        //{
        //    int action = Convert.ToInt32(grdHoliday.Rows[j].Cells[17].Text);
        //    if (action != 0)
        //    {
        //        double exsistingMinu = Convert.ToInt32(grdHoliday.Rows[j].Cells[20].Text);
        ...
        {
            CheckBox chkbox = grdHoliday.Rows[i].FindControl("CheckRet") as CheckBox;
            int action = Convert.ToInt32(grdHoliday.Rows[i].Cells[17].Text);
            if (action != 0)
            {
                double exsistingMinu = Convert.ToInt32(grdHoliday.Rows[i].Cells[20].Text);
                totalMin += exsistingMinu;
            }
            else if (chkbox.Checked == true && action == 0)
            {
                ...
                DateTime dateOutTime = Convert.ToDateTime(((TextBox)(grdHoliday.Rows[i].Cells[8].FindControl("txtntDate1"))).Text == "" ? "" : ((TextBox)(grdHoliday.Rows[i].Cells[8].FindControl("txtntDate1"))).Text + " " + outTime);
                double totalMinutes = DateProcess.GetTotalMinutes(dateInTime, dateOutTime);
                double extraMinutes = 0;
                DateTime startDateForSpecial = new DateTime(2014, 7, 1);
                DateTime endDateForSpecial = new DateTime(2014, 7, 30);
                if (startDateForSpecial <= dateInTime && dateInTime < endDateForSpecial)
                {
                    if (totalMinutes > 390)
                    {
                        extraMinutes = Convert.ToInt32(totalMinutes - 390);
                        grdHoliday.Rows[i].Cells[20].Text = extraMinutes.ToString();
                        grdHoliday.Rows[i].Cells[12].Text = DateProcess.TimeDuration(Convert.ToInt32(extraMinutes));
                    }
                    else
                    {
                        grdHoliday.Rows[i].Cells[12].Text = "00:00";
                ...
                totalMinu = Convert.ToInt32(grdHoliday.Rows[i].Cells[20].Text);
                totalMin += extraMinutes;
            }
            GridViewRow rowFooter = grdHoliday.FooterRow;
            rowFooter.Cells[12].Text = DateProcess.TimeDuration(Convert.ToInt32(totalMin));
        }
    }
}
