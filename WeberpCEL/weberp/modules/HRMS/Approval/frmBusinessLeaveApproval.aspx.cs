...
public partial class modules_HRMS_Approval_frmBusinessLeaveApproval : System.Web.UI.Page
{
    ...
    private void LoadPendingLeaveByemployeeID(string transactionno, string Processid, string flowid,string actingpersonid,string leavecode)
    {
        ...
        cmd.Connection = sqlConn;
        cmd.CommandType = CommandType.StoredProcedure;
        cmd.CommandText = "spProcessGetPendingBusinessLeaveByTransactionno";
    ...
    protected void btnApprove_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            LeaveProcess lvproc = new LeaveProcess();
            string retval = lvproc.SaveApproveBusinessLeaveProcessData(lvphdrlst, myCommand);
            if (retval.ToString() == "")
            {
                myTrans.Rollback("SaveAllTransaction");
            }
    ...
    private void SendMailforForword(int levelid)
    {
        try
        {
            ...
            string msub = "Business Leave application";
            string attn = "Dear sir,";
            string msgbody = "I have sent a busniness leave application for your approval";
    ...
    private void SendMailforReject()
    {
        try
        {
            ...
            string rid = dtr.Rows[0]["Emp_Mas_Remarks"].ToString();
            string rname = dtr.Rows[0]["EmpName"].ToString();
            string msub = "Reject business leave application";
            string msgbody = "Your business leave has been been rejected";
            if (Gender == "M")
                atn = "Dear Mr " + rname + ",";
            else if (Gender == "F")
                atn = "Dear Ms " + rname + ",";
    ...
    private void SendMailforReturn()
    {
        try
        {
            ...
            string rid = dtr.Rows[0]["Emp_Mas_Remarks"].ToString();
            string rname = dtr.Rows[0]["EmpName"].ToString();
            string msub = "Return business leave application";
            string msgbody = "Your business leave has been been returned. Correct your business leave application and send again";
    ...
    private void SendMailforApproval()
    {
        try
        {
            ...
            string rid = dtr.Rows[0]["Emp_Mas_Remarks"].ToString();
            string rname = dtr.Rows[0]["EmpName"].ToString();
            string msub = "Approve business leave application";
            string msgbody = "Your business leave has been approved";
