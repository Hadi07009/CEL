...
public partial class modules_HRMS_Payroll_frmBankAdvice : System.Web.UI.Page
{
    ...
    protected void btnBankAdvice_Click(object sender, EventArgs e)
    {
        ...
        if (rblForCondition.SelectedValue == "P")
            ...
            DataProcess.ExecuteQuery(Session[GlobalData.sessionConnectionstring].ToString(), "Create view salarypostedview as select * from Hrms_salary1 where month(salmonth)=" + mth + " and year(salmonth)=" + yr + " and Salgrade<>'50' and empcode in(select Emp_ID from Hrms_Emp_Bnk_Info where Acc_No<>'')");
