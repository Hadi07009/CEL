...
public partial class modules_HRMS_Payroll_frm_Bonus_Cyclic : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        GlobalData.ConfirmBox(btnCreatePayroll, "Are you sure to create bonus ?");      
    ...
    protected void ddlcompany_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        try
        {
            DataTable dtForLastSal = DataProcess.GetData(ConnectionString, Sqlgenerate.SqlGetLastBonusMonth());
            ...
            TextBox1.Text = string.Format("{0:00}", DateTime.Now.Day) + "/" + string.Format("{0:00}", DateTime.Now.Month) + "/" + DateTime.Now.Year.ToString();
        }
        catch
        {
    ...
    protected void btnpost_Click(object sender, EventArgs e)
    {
        ...
        if (DataProcess.GetData(Session[GlobalData.sessionConnectionstring].ToString(), SqlgenerateForPayroll.GetBonusDataForPost(dbname, Convert.ToDateTime(dtForSal1.Rows[0][1].ToString()).Month, Convert.ToDateTime(dtForSal1.Rows[0][1].ToString()).Year)).Rows.Count > 0)
        {
            MessageBox1.ShowWarning("Bonus already posted. So you can not re posted");
        ...
        try
        {
            DataProcess.DeleteQuery(Session[GlobalData.sessionConnectionstring].ToString(), SqlgenerateForPayroll.DeletePostedBonus(Convert.ToDateTime(dtForSal1.Rows[0][1].ToString()).Month, Convert.ToDateTime(dtForSal1.Rows[0][1].ToString()).Year));
            DataProcess.InsertQuery(Session[GlobalData.sessionConnectionstring].ToString(), SqlgenerateForPayroll.BonusPost());
            transactionString.Commit();
        }
        catch (Exception ex)
        {
        ...
        {
            //lblmsg.Text = (ts.Minutes * 60 + ts.Seconds).ToString() + " Seconds";
            MessageBox1.ShowSuccess("Bonus Post Successful");
    ...
    protected void btnCreatePayroll_Click(object sender, EventArgs e)
    {
        ...
        int mm = 0, yyyy = 0;
        string[] tmp = TextBox1.Text.Split('/');
        mm = Convert.ToInt32(tmp[1]);
        yyyy = Convert.ToInt32(tmp[2]);

        if (SalaryPostedValidation(mm, yyyy) == false)
        {
            MessageBox1.ShowWarning("Already bonus has been posted for this period.\nPlease check the period");            
            return;
        }
        ...
        {            
            DateTime stdate, enddate;
            stdate = DateProcess.FirstDateOfMonth(Convert.ToDateTime(TextBox1.Text));
            enddate = Convert.ToDateTime(TextBox1.Text);
        
            cmd.Connection = sqlConn;
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.CommandText = "spBonusCreate";
            ...
            MessageBox1.ShowSuccess("Bonus calculate successful.");  
    ...
    private bool SalaryPostedValidation( int mm,int yyyy)
    {
        ...
        try
        {
            if (DataProcess.GetData(Session[GlobalData.sessionConnectionstring].ToString(), SqlgenerateForPayroll.GetBonusDataForPost(dbname, mm, yyyy).ToString()).Rows.Count > 0)
            {
