...
public partial class modules_HRMS_Payroll_frmPaySetup : System.Web.UI.Page
{
    ...
    private double GetMonthlyTaxAmount(string detEmpId)
    {
        ...
        DateTime salmonth;
        DataTable dt = new DataTable();
        dt = DataProcess.GetData(Session["ConnectionStr"].ToString(), Sqlgenerate.GetMaxSalDate());

        salmonth = Convert.ToDateTime(dt.Rows[0]["NextSaldate"].ToString());


        DataTable dttax = DataProcess.GetData(Session["ConnectionStr"].ToString(), Sqlgenerate.GettaxbyempidandSalmonth(detEmpId, salmonth));
