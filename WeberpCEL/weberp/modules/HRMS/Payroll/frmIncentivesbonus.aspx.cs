...
public partial class modules_HRMS_Payroll_frmIncentivesbonus : System.Web.UI.Page
{
    ...
    protected void btnUploadTaxChallan_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            MessageBox1.ShowSuccess("Incentive Bonus Update Successful.");
        }
    ...
    private void UploadTaxChallanData()
    {
        ...
        string oFilename = "Incentivesbonus-" + mm + yyyy;
        ...
        string sql = "delete from HrmsOthersPaymentwithsalary where month(oSalmonth)=month(Convert(Datetime,'" + saldate + "',103)) and year(oSalmonth)=year(Convert(Datetime,'" + saldate + "',103)) and [Type] in('IB')";
        ...
        foreach (DataRow dtRow in dtFromExcel.Rows)
        {
            ...
            _objOvertime.ChallanDate = saldate.ToString();
            _objOvertime.ChallanForMonth = saldate.ToString();
            _objOvertime.ChallanNumber = "0";
            _objOvertime.ChallamAmount = dtRow["Incentive Amount"].ToString() == "" ? 0 : Convert.ToDecimal(dtRow["Incentive Amount"].ToString());
            _objOvertime.Paytype = "IB";

            if (_objOvertime.ChallamAmount>0) 
