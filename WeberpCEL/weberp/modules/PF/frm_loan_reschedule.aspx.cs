...
public partial class frm_loan_reschedule : System.Web.UI.Page
{
    ...
    protected void btnshow_Click(object sender, EventArgs e)
    {
        ...
        if (txtemployee.Text == "")
        {
            lblMessage.Text = "Please enter valid employee code / Loan refno.";
        ...
        loan_ref = txtemployee.Text.Split(':')[0];
        emp_code = txtemployee.Text.Split(':')[1];
    ...
    protected double calcMonthly(double principalAmt, double noOfPayment, double iRate)
    {
        ...
        a = (Math.Pow((1 + intRate), noOfPayment));
        b = (Math.Pow((1 + intRate), noOfPayment) - 1);
        if (b == 0) b = 1;

        if (intRate == 0) intRate = 1 / noOfPayment;
        monthly = principalAmt * a * intRate / b;
        return Convert.ToDouble(monthly);
    ...
    private string  check_entry()
    {
        ...
        if (txtnoofinst.Text == "" || txtnoofinst.Text == (0).ToString()) return "Please enter no of intallment greater than 0";
        return "";
    }
